resource "google_cloud_run_service" "probe-errors-fail-service" {
  ...
  template {
    spec {
      containers {
        image = data.google_container_registry_image.probe_errors_fail_latest.image_url
        env {
          name  = "PROJECT_ID"
          value = "dlorch-bd021"
        }
...
data "docker_registry_image" "probe_errors_fail_image" {
  name = "gcr.io/dlorch-bd021/probe-errors-fail"
}

data "google_container_registry_image" "probe_errors_fail_latest" {
  name    = "probe-errors-fail"
  project = "dlorch-bd021"
  digest  = data.docker_registry_image.probe_errors_fail_image.sha256_digest
}
